    ...
    <div class="flex flex-col md:flex-row gap-4">
              ...
              @forelse ($cart_items as $item)
                <tr wire:key='{{ $item['product_id'] }}'>
                  ...
                  <td><button wire:click="removeItem({{ $item['product_id'] }})"
                      class="bg-neutral-200 border-2 border-neutral-300 rounded-lg px-3 py-1 hover:bg-red-500 hover:text-white hover:border-red-700">
                      <span wire:loading.remove wire:target="removeItem({{ $item['product_id'] }})">Remove</span><span
                        wire:loading="removeItem({{ $item['product_id'] }})">Removing...</span>
                    </button></td>
                </tr>
              @empty
                <tr>
                  <td colspan="5" class="text-center py-4 text-4xl font-semibold text-neutral-500">Your cart is empty.
                  </td>
                </tr>
      ...
      <div class="md:w-1/4">
        <div class="bg-neutral-700 rounded-lg shadow-md p-6">
          ...
          @if($cart_items)
            <a href="/checkout"
              class="bg-brand-600 block text-center text-white py-2 px-4 rounded-lg mt-4 w-full hover:bg-brand-700">Checkout</a>
          @endif
        </div>
      </div>
    </div>
